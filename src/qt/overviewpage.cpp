...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::TSC)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTestcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTestcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTestcoinAmount = strAnonymizeTestcoinAmount.remove(strAnonymizeTestcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTestcoinAmount * COIN) nMaxToAnonymize = nAnonymizeTestcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTestcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTestcoinAmount));
        strAnonymizeTestcoinAmount = strAnonymizeTestcoinAmount.remove(strAnonymizeTestcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
